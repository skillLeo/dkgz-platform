...
/** Wording from the admin panel, with the built-in text as the fallback. */
const t = (field, fallback = '') => page.props.app?.cookie_notice?.[field] || fallback
...
<template>
    ...
    >
        <div class="mx-auto flex max-w-(--container-wide) flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="min-w-0">
                <p id="cookie-titel" class="text-base font-medium text-navy-700">
                    {{ t('titel', 'Statistik-Cookies') }}
                </p>
                <p id="cookie-text" class="measure pt-1 text-sm leading-normal text-gray-600">
                    {{ t('text', 'Wir möchten anonym messen, wie diese Seite genutzt wird. Für den Betrieb notwendige Cookies setzen wir ohnehin — dafür ist keine Zustimmung nötig. Sie können jederzeit widersprechen.') }}
                    <a href="/datenschutz" class="font-medium text-navy-700 underline underline-offset-2">
                        {{ t('link', 'Datenschutzerklärung') }}
                    </a>
                </p>
            </div>
            <div class="flex shrink-0 flex-wrap gap-3">
                <BaseButton variant="secondary" size="compact" @click="decide(false)">
                    {{ t('button_ablehnen', 'Nur notwendige') }}
                </BaseButton>
                <BaseButton size="compact" @click="decide(true)">
                    {{ t('button_annehmen', 'Einverstanden') }}
                </BaseButton>
            </div>
        </div>
    </div>
